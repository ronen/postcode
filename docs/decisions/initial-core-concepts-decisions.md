...
Before this decision, the canonical core-concepts document defined its purpose but contained no adopted definitions. Terminology already appears in the adopted product design and accepted decisions, with descriptive architecture and one implemented TypeScript slice supplying examples. Later work needs a concise shared vocabulary without turning that slice's record shapes into the conceptual model of every language or analysis.

This bundle establishes the initial definitions in the governing [core-concepts document](../core-concepts.md) and records their rationale. It does not authorize implementation, alter the adopted product design, or replace the binding architectural constraints. No existing accepted decision is superseded: the definitions consolidate established distinctions, and the clarification of Property and Facet is identified below rather than presented as a previously adopted formal model.

### Source basis

The adopted design governs product meaning; accepted decisions establish scoped architectural choices. The [approved initial module inventory plan](../plans/initial-module-inventory-plan.md#success-criteria) corroborates how those choices fit together. The [current architecture overview](../architecture/README.md) describes their realization. Implementation was inspected only as supporting evidence, not as authority for filling semantic gaps.

| Terminology | Principal basis and limit |
| --- | --- |
| Entity, Subject, identity and names | Product design [identity and continuity](../../foundation/product-design.md#24-identity-and-continuity-of-attention), [naming](../../foundation/product-design.md#25-naming-and-terminology), [navigation](../../foundation/product-design.md#34-navigation-through-views), and [summary subjects](../../foundation/product-design.md#411-summary-lenses); accepted [domain entities](initial-module-inventory-decisions.md#represent-modules-as-qualified-domain-entities). Subject is an investigative role, not a promise that every such subject is currently selectable. |
| Claim, Relationship, Claim context | Accepted [record-oriented model](initial-projection-architecture-decisions.md#use-a-record-oriented-program-information-model), [domain entities](initial-module-inventory-decisions.md#represent-modules-as-qualified-domain-entities), and [Claim context and evaluation outcomes](initial-module-inventory-decisions.md#preserve-claim-context-and-evaluation-outcomes-distinctly). The sources distinguish addressable categories without establishing an exhaustive taxonomy. |
| Property, Facet | Product design [epistemological contract](../../foundation/product-design.md#22-epistemological-contract) and [test lenses](../../foundation/product-design.md#414-test-lenses) use property descriptively. Accepted [module facets](initial-module-inventory-decisions.md#represent-modules-as-qualified-domain-entities) and [conceptual presentation and source escape](initial-module-inventory-decisions.md#keep-conceptual-presentation-separate-from-source-escape) establish overlapping facets. Human-supplied review feedback informed the further clarification that Facet is a classification role played by a property, without special epistemological strength or a universal representation. |
| Evidence and qualification | Product design [sources of program knowledge](../../foundation/product-design.md#23-sources-of-program-knowledge); accepted [qualification constraints](adopt-qualification-and-evaluation-constraints.md#preserve-claim-qualification-throughout-processing-and-presentation) and [observation production](initial-observation-recording-decisions.md#record-normal-view-production-automatically). Evidence is broader than source locations or proof. |
| Evaluation, outcome, materialization | Product design [resource-bounded analysis](../../foundation/product-design.md#2151-resource-bounded-analysis); accepted [evaluation separation](initial-projection-architecture-decisions.md#separate-lens-requirements-evaluation-and-projection-construction) and [evaluation distinctions](adopt-qualification-and-evaluation-constraints.md#keep-evaluation-materialization-and-failure-distinctions-explicit). No scheduler or fixed state schema is implied. |
| Lens, Projection, Presentation, View, Expansion | Product design [four-way distinction](../../foundation/product-design.md#21-lenses-projections-presentations-and-views) and [navigation](../../foundation/product-design.md#34-navigation-through-views); accepted [projection records](initial-projection-architecture-decisions.md#make-projection-and-evaluation-state-first-class-records), [standard expansions](initial-module-inventory-decisions.md#represent-modules-as-qualified-domain-entities), and [source detail](initial-module-inventory-decisions.md#keep-conceptual-presentation-separate-from-source-escape). Logical distinctions do not prescribe physical execution stages. |
| Analysis snapshot | Accepted [logical identity](initial-projection-architecture-decisions.md#use-deterministic-logical-identity-independently-of-persistence), [snapshot-scoped references](initial-module-inventory-decisions.md#make-references-repeatable-but-snapshot-scoped), and [analysis versus invocation identity](adopt-identity-evidence-and-observation-constraints.md#make-analysis-identity-independent-of-invocation-identity). This is analysis context, not a cache-validity or atomic-capture definition. |
...
### Define Facet as a classification role played by a property

#### Decision

Use Property for a characteristic of a subject about which information can be requested or asserted. Distinguish that characteristic from a claim about its value or whether it holds. Use Facet for a property used as a compact classification dimension for describing, filtering, grouping, or comparing entities. A claim supplies its value, and Claim context supplies its qualification. Different facets can overlap and need not share one representation or value type.

Facet is a role played by a property, not a separate record category or a special epistemological status. Preserve the accepted distinction between conceptual and source facets. Adopt no universal Property or Facet schema, implementation subtype hierarchy, or generic facet machinery.

#### Rationale

The product design asks about properties and uses a behavioral property in its test example. The module decisions define facets as overlapping established characteristics and distinguish conceptual information from source detail. Defining Facet only by being established and describing an entity does not explain its classification role and risks implying that the term confers stronger truth status.

The classification role provides the missing distinction while leaving claim strength to the evidence, method, and Claim context. It accommodates both the initial slice's tag facets and an enumerated property used for classification, such as the `modulePresence` example supplied in review. That example illustrates compatibility, not an adoption of Slice 2 semantics or a new value schema. A property can serve as a facet without acquiring a separate record or a common value type with every other facet.
...
- Define Facet only as an established characteristic, leaving its relationship to Property unresolved: does not explain what makes it a facet and can suggest a special epistemological strength.
- Make Facet a mutually exclusive entity kind: contradicts accepted overlap and confuses classification with entityhood.
- Require a separate Facet record category, implementation subtype, or shared value schema: adds representation commitments that the classification role does not require.

#### Consequences

Property can be used in planning without implying an API, and Facet identifies how a property is used. Facet does not acquire the current module facet list as a universal enumeration or confer stronger qualification on a claim. Property identity, addressability, representation, and generic facet machinery remain unresolved. Any such machinery requires a further choice rather than an inference from these definitions.
...
These are retained limits of this decision. Adoption of the terminology does not settle a formal model that no approved work currently requires.

1. **Property identity and facet machinery.** This decision defines Facet as a classification role played by a property and distinguishes the property from a claim about its value. Property granularity, identity, addressability, and representation remain unspecified, as do generic mechanisms for declaring or using facets. The conceptual relationship is adopted; it does not settle an implementation subtype hierarchy, shared value type, registry, or query API.

2. **Claim and recorded assertion.** The accepted module decision defines Claim as information asserted by an analysis result, while the product design discusses claims with recorded-assertion status and the record-model decision names claims and recorded assertions separately. These sources clearly distinguish the statement's contents, the fact that it was recorded, and its association with a subject. They do not settle whether all recorded assertions are formally Claim subtypes. In the supporting [record definitions](../../src/lib/records.ts), `RecordedAssertion` is outside the `Claim` union and its associated context currently has a mechanically-derived discriminator. That is not sufficient to decide the general taxonomy or to declare the documentation's contents mechanically established. This decision preserves the semantic distinction without prescribing union membership or moving qualification fields.
...
Resolve the questions above when later work depends on a stronger formal model, particularly before relying on projection identity across evaluation attempts, presentation-driven materializations, or durable reuse. Such work requires a further scoped decision; this adoption does not authorize implementation.
