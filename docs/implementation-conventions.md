...
- Retain provider discovery and completed work by declared requirements. Keep the module evaluation basis and its expansions together; never broaden projections by selecting the accumulated store indiscriminately. Require completed, fully materialized requested expansions as well as the root module outcome before caching a module basis, including a dependency request’s module basis. Retry partial expansions without replacing their earlier records. [[Accumulated information](decisions/transient-analysis-sessions.md#immutable-information-within-an-accumulating-session)]
