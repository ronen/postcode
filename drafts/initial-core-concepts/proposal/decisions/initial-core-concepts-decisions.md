...
This proposed bundle accompanies the complete proposed [core-concepts document](../docs/core-concepts.md). Both remain non-governing drafts. Acceptance would populate that document and record the rationale together. It would not authorize implementation, alter the adopted product design, or replace the binding architectural constraints. No existing accepted decision is proposed for supersession: the definitions consolidate established distinctions, and the proposed clarification of Property and Facet is identified below rather than presented as a previously adopted formal model.
...
1. **Property identity and facet machinery.** The proposal defines Facet as a classification role played by a property and distinguishes the property from a claim about its value. Property granularity, identity, addressability, and representation remain unspecified, as do generic mechanisms for declaring or using facets. The conceptual relationship is proposed for adoption; it does not settle an implementation subtype hierarchy, shared value type, registry, or query API.
