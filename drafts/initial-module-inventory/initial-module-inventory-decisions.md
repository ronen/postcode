...
- Use paths as module identity or primary labels: rejected because PostCode
  navigation is entity-oriented and source paths are escape-hatch detail.
- Infer entry modules from package metadata or layout: rejected because
  `modules(project)` inventories the configured Program and does not claim an
  executable application root.
- Treat documentation as derived truth: rejected because it is a recorded
  assertion even when mechanically associated with an entity.
