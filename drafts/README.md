...
The human directs promotion, reclassification, and disposal. Move or incorporate approved plans into [`docs/plans/`](../docs/plans/), accepted decisions into [`docs/decisions/`](../docs/decisions/), and other material into its appropriate canonical location. Human-directed exploratory material that should remain available without approval may instead be reclassified under [`notes/`](../notes/). Review promoted content against the destination's requirements; carry all context needed to understand it into canonical documents rather than citing draft artifacts. An abandoned draft may be deleted without receiving a lifecycle status; Git retains its history.
