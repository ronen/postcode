...
- Application modules outside `src/lib/typescript/` must not import the `typescript` package. Interfaces between the TypeScript integration and the rest of the application use PostCode domain types rather than exposing TypeScript compiler nodes, symbols, or other compiler objects. Tests may import the compiler directly when they need to verify the integration against real compiler behavior. [[TypeScript language-integration boundary](../../../docs/decisions/initial-module-inventory-decisions.md#begin-with-a-typescript-module-inventory)]
...
## Analysis methods and record identity

- Version analysis methods when their semantics change, retaining the responsible method and version with evidence and results. Session identity does not substitute for method provenance. [[Accumulated results](../decisions/transient-analysis-sessions.md#retain-results-and-coordinate-additional-analysis)]
- Allocate references without changing an existing binding when further entities or results are added. Do not derive the active reference namespace from a digest of the growing input collection. Identifier allocation mechanics remain implementation choices. [[Session references](../decisions/transient-analysis-sessions.md#bind-references-within-the-active-session)]
- Compare semantic content and relationships in equivalent-run tests with explicit normalization of session-local identities. Do not normalize away evidence, qualification, selection, or meaningful differences.

## Selection and navigation references

- Inspection accepts one exact referent: a name, handle, or Entity ID. One name or handle may resolve to zero, one, or multiple subjects. Multiple input referents and list-selector syntax remain deferred.
- Keep exact name and generated-handle lookups distinct from bound precise references. Provide an unambiguous way to request each when spellings collide; the concrete syntax remains to be settled before implementation. [[Session references](../decisions/transient-analysis-sessions.md#bind-references-within-the-active-session)]
- Generate mnemonic handles from language names, extensionless source basenames, or declared exports. Retain generated status and provenance; a basename cue is not a conceptual name or responsibility classification. Generic basenames fall back to representative exports or honest anonymity. Preserve separation between generated cues and the chosen precise-reference syntax.
- Groups have intrinsic segment names (the repository root has none), and no generated handles or path selectors. Organization inspection selects groups and modules together by exact name, or precisely by an active-session Entity ID.
- Keep internal record keys, public Entity IDs, names, and handles distinct. An unknown precise reference does not fall back to a name, handle, path, fuzzy match, or inferred successor. No cross-invocation reference contract is provided. [[Session references](../decisions/transient-analysis-sessions.md#bind-references-within-the-active-session)]

## CLI operation

- Do not generate next-command text in views. Document invocation and navigation in help and the CLI reference.
...
- Write each self-contained command observation batch as one JSON file beneath a UTC date directory named `date=YYYY-MM-DD` in the PostCode development checkout's ignored `_observations/` directory. Begin each filename with the filesystem-safe UTC timestamp `timestamp=YYYY-MM-DDTHH-MM-SS.sssZ_` and follow it with the batch UUID.
- Use the PostCode working tree containing the running CLI build as the local sink destination, independently of the selected project's configuration directory. Do not send observations to a remote or shared sink under the current configuration. [[Send to a sink and forget](../../../docs/decisions/initial-observation-recording-decisions.md#send-to-a-sink-and-forget)]
...
- Organization/group views use the experimental `postcode-organization-view/0-experimental` schema; module-only views retain the existing schema. Mixed inspection embeds module detail and sections the subject kinds. Declare group details and module-standard expansions before evaluation. Coordinate requested analysis through the session; rendering consumes materialized records only.
...
- Expose source detail only through an explicit inspection expansion and group it by conceptual module and export labels. [[Conceptual presentation and source escape](../../../docs/decisions/initial-module-inventory-decisions.md#keep-conceptual-presentation-separate-from-source-escape)]
- Preserve module and export containment in source presentation, and distinguish forwarding declarations from semantic-symbol definitions. [[Conceptual presentation and source escape](../../../docs/decisions/initial-module-inventory-decisions.md#keep-conceptual-presentation-separate-from-source-escape)]
